...
当前聊天页支持对已完成消息执行复制、下载、编辑、删除、重新发送和重新生成。`streaming` 状态的消息不能编辑或删除。

### 重新发送

指编辑某条用户消息后，从这条用户消息重新开始生成回答。

系统会删除这条用户消息之后的旧会话消息，再创建新的助手回复。

### 重新生成

指对某条助手消息重新生成回答。

系统会查找该助手消息之前最近的一条用户消息，并复用原助手消息 ID 更新内容。
...
该提示用于用户界面和业务引导。新助手消息不再把它写入正文；读取历史旧消息时仍会剥离该提示，避免把它当成真实问答内容反复累积。
...
其中 `system` 表示跟随操作系统设置。该偏好保存在 `User.colorMode`，与侧边栏主题 `User.sidebarTheme` 分开。
