...
- 一键转人工工单
- 部门工单实时待办提醒与浏览器通知
- 工单自动分发到部门，部门人员可在未认领时直接流转到其他部门，也可认领后回复、转派、提交处理方案
- 药店工作人员确认问题已解决后，部门人员先生成待入库知识草稿，再关闭工单写回知识库
- 历史会话支持软删除
- 统计指标、历史问答、历史工单、近 7 天趋势
- 知识库支持上传 Word/PDF/Markdown/TXT/图片，按“业务分类”和通用/湖北城市范围管理文档与 chunk
- RAG 支持向量召回、BM25 关键词召回、HQ 检索投影、RRF 合并、多模态 rerank 和索引任务重试
...
├─ 信息部常见问题详解.doc
├─ docker-compose.yml
├─ docker-compose.v2.yml     # v2 隔离运行编排
├─ docker-compose.v21.yml    # v2.1 隔离运行编排
├─ .env.example
└─ README.md
```
...
- 自动生成工单
- 按问题内容自动分发到匹配部门；无法判断时进入“其他部门”
- 目标部门人员可在 `/department/tickets` 的待认领列表看到工单
...
### 场景 6：员工确认解决，部门人员生成待入库知识并关闭

- 药店工作人员在工单详情确认问题已解决
- 当前处理部门人员选择材料生成待入库知识
- 当前处理部门人员、提交工单的员工或管理员关闭工单
...
- 用新工单
- 当前处理部门人员点击转派并选择目标部门或目标人员
- 切换目标部门专家处理

预期：

- 工单状态变为 `escalated`
- 工单出现在目标专家 `/department/tickets` 列表
- 统计页对应处理数据增加
...
- 部门工单页面左侧能看到待处理工单数量
- 新建工单或转派工单后，部门侧无需刷新即可看到数量变化和站内提醒
- 浏览器授权通知后，可收到系统通知
