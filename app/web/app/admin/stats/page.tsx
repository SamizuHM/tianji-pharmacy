...
export default async function AdminStatsPage(props: {
  ...
  const llmRate = summary.totalQuestions
    ? Math.round((summary.llmAnswers / summary.totalQuestions) * 1000) / 10
    : 0;
  const transferRate = summary.totalQuestions
    ? Math.round((summary.transferCount / summary.totalQuestions) * 1000) / 10
    : 0;
          ...
          <CardContent>
            <div className="flex items-center justify-center gap-8">
              ...
              <div className="flex flex-col gap-3 text-sm">
                ...
                <Legend
                  color="bg-purple-500"
                  label={`大模型回答 ${summary.llmAnswers} (${llmRate}%)`}
                />
                <Legend
                  color="bg-slate-300"
                  label={`转人工次数 ${summary.transferCount} (${transferRate}%)`}
                />
              </div>
            </div>
            <div className="mt-6 border-t border-border pt-4 text-sm text-muted">
              知识库命中率 <span className="font-semibold text-teal-600">{kbRate}%</span>
              ，工单闭环率{" "}
              <span className="font-semibold text-emerald-600">
                {Math.round(summary.closedRate * 1000) / 10}%
              </span>
            </div>
          </CardContent>
        </Card>
            ...
            <Table>
              ...
              <TBody>
                {messages.items.map((message) => (
                  <tr key={message.id}>
                    <TD>
                      <div className="max-w-[260px] truncate font-medium text-slate-900 dark:text-foreground">
                        {message.contentText}
                      </div>
                    </TD>
