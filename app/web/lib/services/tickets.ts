...
export async function closeTicketWithKnowledgeWriteback(input: {
  ...
  let item;
  if (input.existingKnowledgeItemId) {
    const { updateKnowledgeItem } = await import("@/lib/services/knowledge");
    item = await updateKnowledgeItem(input.existingKnowledgeItemId, {
    ...
    item = await upsertKnowledgeItem({
      categoryL1: draft.categoryL1,
      categoryL2: draft.categoryL2,
      question: draft.question,
      answer: draft.answer,
