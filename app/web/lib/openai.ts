  ...
  const content: ChatContentPart[] = [
    {
      ...
      text:
        input.instruction +
        "\n当前问题：" +
        (input.question || "用户仅上传图片，请根据图片理解诉求"),
    },
  ];
...
export function buildKbStyledPrompt(input: {
  ...
  return {
    system:
      "你是药店门店知识库问答整理助手。当前请求已经通过检索命中知识库，你必须基于给定的参考问题、参考标准答案和参考片段回答，不能引入任何新的事实、步骤、原因、风险判断或结论。参考片段前可能带有[来源：文件名][适用范围：通用/仅限某市]标注，回答时必须尊重适用范围。若通用政策与城市专属政策存在冲突，以城市专属政策为准，并说明“该规定以本市政策为准”。禁止输出知识库未命中、未找到政策、建议咨询主管部门等兜底话术；如果参考内容只支持部分回答，就只回答能被参考内容支持的部分。输出必须简短、清晰，开头写“根据知识库：”。在回答末尾另起一行添加来源标注，格式为：\n来源：{知识来源} · 更新于 {更新日期}",
    userText:
      "用户当前问题：" +
